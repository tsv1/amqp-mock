...
AnyFrame = Union[base.Frame, ContentHeader, ContentBody]
...
class AmqpConnection:
    ...
    async def _send_channel_open_ok(self, channel_id: int, frame_in: commands.Channel.Open) -> None:
        frame_out = commands.Channel.OpenOk()
        await self._send_frame(channel_id, frame_out)

    async def _send_queue_declare_ok(self, channel_id: int, frame_in: commands.Queue.Declare) -> None:
        frame_out = commands.Queue.DeclareOk(queue=frame_in.queue, message_count=0, consumer_count=0)
        return await self._send_frame(channel_id, frame_out)
    ...
    async def _send_channel_close_ok(self, channel_id: int, frame_in: commands.Channel.Close) -> None:
        frame_out = commands.Channel.CloseOk()
        await self._send_frame(channel_id, frame_out)

    async def _send_basic_qos_ok(self, channel_id: int, frame_in: commands.Basic.Qos) -> None:
        frame_out = commands.Basic.QosOk()
        return await self._send_frame(channel_id, frame_out)

    async def _send_basic_cancel_ok(self, channel_id: int, frame_in: commands.Basic.Cancel) -> None:
