...
@pytest.mark.asyncio
async def test_reset_exchanges(*, amqp_mock_server, amqp_mock_client, amqp_client):
    with given():
    ...
    with when():
        result = await amqp_mock_client.reset()
...
@pytest.mark.asyncio
async def test_reset_queues(*, amqp_mock_server, amqp_mock_client, amqp_client):
    with given():
        queue = "test_queue"
...
@pytest.mark.asyncio
async def test_reset_history(*, amqp_mock_server, amqp_mock_client, amqp_client):
    with given():
        queue = "test_queue"
        await amqp_mock_client.publish_message(queue, Message("text"))
        await amqp_client.consume(queue)
