...
@pytest.mark.asyncio
async def test_publish_single_message(*, amqp_mock_server, amqp_mock_client, amqp_client):
    with given():
        queue = "test_queue"
        message = "text"
...
@pytest.mark.asyncio
async def test_publish_multiple_messages(*, amqp_mock_server, amqp_mock_client, amqp_client):
    with given():
        queue = "test_queue"
        message1, message2 = "text1", "text2"
        await amqp_mock_client.publish_message(queue, Message(message1))

    with when():
        await amqp_mock_client.publish_message(queue, Message(message2))
...
@pytest.mark.asyncio
async def test_publish_message_specific_queue(*, amqp_mock_server, amqp_mock_client, amqp_client):
    with given():
        queue1, queue2 = "test_queue1", "test_queue2"
        message1, message2 = "text1", "text2"
        await amqp_mock_client.publish_message(queue1, Message(message1))

    with when():
        await amqp_mock_client.publish_message(queue2, Message(message2))
...
@pytest.mark.asyncio
async def test_publish_no_messages(*, amqp_mock_server, amqp_mock_client, amqp_client):
    with given():
        queue = "test_queue1"
