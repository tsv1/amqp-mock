...
@pytest.mark.asyncio
async def test_delete_exchange_message(*, amqp_mock_server, amqp_mock_client, amqp_client):
    with given():
    ...
    with when():
        result = await amqp_mock_client.delete_exchange_messages(exchange)
...
@pytest.mark.asyncio
async def test_delete_no_exchange_messages(*, amqp_mock_server, amqp_mock_client, amqp_client):
    with given():
        exchange = "test_exchange"
